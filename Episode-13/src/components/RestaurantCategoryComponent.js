...
const RestaurantCategoryComponent =({data, showItems, setShowIndexToShowCategory,dummy})=>{
        ...
        <div>
            ...
            {   //If showItems is true then only render ItemListComponent
               //if showItems is false then do not render ItemListComponent
                showItems &&  <ItemListComponent items={data.categories[0].category.items} dummy={dummy}/>
            }
            </div>
        </div>
    );
